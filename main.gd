...
# ---------------------------------------------------------------------------
# Cardinal direction discs (45 m out in each compass direction)
# ---------------------------------------------------------------------------
func _setup_compass_discs() -> void:
	_add_compass_disc(Vector3(  0.0, 0.01, -45.0), Color(0.90, 0.90, 1.00))  # N = −Z
	_add_compass_disc(Vector3(  0.0, 0.01,  45.0), Color(1.00, 0.80, 0.80))  # S = +Z
	_add_compass_disc(Vector3( 45.0, 0.01,   0.0), Color(1.00, 0.70, 0.30))  # E = +X
	_add_compass_disc(Vector3(-45.0, 0.01,   0.0), Color(0.70, 1.00, 0.70))  # W = −X


func _add_compass_disc(pos: Vector3, color: Color) -> void:
...
# ---------------------------------------------------------------------------
# Reference pole grid (9 × 9 at 20 m spacing, centre skipped)
# ---------------------------------------------------------------------------
func _setup_poles() -> void:
	var palette := [
		Color(1.00, 0.18, 0.18), Color(0.18, 0.38, 1.00),
		Color(1.00, 0.85, 0.08), Color(0.10, 0.82, 0.38),
		Color(0.88, 0.22, 1.00), Color(0.08, 0.90, 0.88),
		Color(1.00, 0.48, 0.08), Color(1.00, 0.38, 0.65),
	]

	for xi: int in range(-4, 5):
		for zi: int in range(-4, 5):
			...
			mi.mesh              = cyl
			mi.material_override = mat
			mi.position          = Vector3(xi * 20.0, 5.0, zi * 20.0)
			add_child(mi)
			...
			cap_mi.position          = Vector3(xi * 20.0, 10.15, zi * 20.0)
			add_child(cap_mi)


# ---------------------------------------------------------------------------
# Scattered cubes and spheres (near spawn area for scale reference)
# ---------------------------------------------------------------------------
func _setup_scatter() -> void:
	...
	var palette := [
		Color(0.90, 0.10, 0.10), Color(0.10, 0.25, 0.90),
		Color(0.90, 0.80, 0.08), Color(0.10, 0.80, 0.28),
		Color(0.80, 0.10, 0.90), Color(0.08, 0.90, 0.90),
		Color(0.90, 0.48, 0.08), Color(0.90, 0.28, 0.60),
	]
